...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
    {
        ...
        public void ReadReport(ReadReportCallback callback, int timeout)
        {
            ...
            var workTask = Task.Run(() => readReportDelegate.Invoke(timeout));
            workTask.ContinueWith(EndReadReport);
            workTask.Wait();
        }
        ...
        public void WriteReport(HidReport report, WriteCallback callback, int timeout)
        {
            ...
            var workTask = Task.Run(() => writeReportDelegate.Invoke(report, timeout));
            workTask.ContinueWith(EndWriteReport);
            workTask.Wait();
        }
        ...
        protected void EndReadReport(IAsyncResult ar)
        {
            var task = (Task<HidReport>)ar;
            if (_callback2 != null)
                _callback2.Invoke(task.Result);
        ...
        private void EndWriteReport(IAsyncResult ar)
        {
            var task = (Task<bool>)ar;
            if(_callback != null)
